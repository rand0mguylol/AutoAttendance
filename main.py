...
def screenshot(application_name: str = "Microsoft Teams"):
    ...
    window = [(hwnd, title) for hwnd, title in winlist if
              application_name.lower() in title.lower() and "notification" not in title.lower()]
    if len(window) < 1:
...
while True:
    ...
    try:
        ...
        logging.info(f"Error encountered trying to screenshot: {e}")
